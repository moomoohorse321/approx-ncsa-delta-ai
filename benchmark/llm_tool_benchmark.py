...
# Local benchmark imports (decoupled from legacy LLM_tool/).
try:
    ...
    # Fallback for direct execution from benchmark/ directory.
    from questions import Question, evalutation_questions, questions_to_run  # noqa: E402
    from llm import LLMManager
    import tool_runtime_common as trc
...
from benchmark_common import default_toolchain  # noqa: E402

ToolSpec = trc.ToolSpec
ToolRuntime = trc.ToolRuntime
_tokenizer = None
...
def _measure_llm_call(llm: LLMManager, prompt: str, model_size: str) -> Tuple[str, float]:
    if getattr(llm, "backend", "iree") == "iree":
        ...
        return output, _get_total_compute_ms()
...
def _build_tools(
    ...
    return trc.build_tools(
        toolchain=toolchain,
        cgeist_config=cgeist_config,
        exec_root_base=Path("/u/haor2/workloads/benchmark/artifacts"),
    )
