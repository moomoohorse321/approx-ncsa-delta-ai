...
from gemma.gm.iree.text import _sampler_loop
from gemma.gm.iree.text import _turn_utils
from gemma.gm.typing import _common
from gemma.gm.utils import _cache_helper
from gemma.gm.utils import _types
from gemma.gm.iree import _iree
import jax
...
def _mlir_paths_for_model(model: _transformer.Transformer) -> tuple[str, str]:
  """Pick MLIR artifacts based on model config."""
  embed_dim = model.config.embed_dim
  if embed_dim == 1152:
    return ("gemma_1b_2048_prefill.mlir", "gemma_1b_2048_decode.mlir")
  if embed_dim == 2560:
    return ("gemma_4b_2048_prefill.mlir", "gemma_4b_2048_decode.mlir")
  raise ValueError(f"Unsupported embed_dim for MLIR paths: {embed_dim}")
...
def prefill(
  ...
  prefill_path, decode_path = _mlir_paths_for_model(model)
  out = _iree.get_iree_runner(
      params, prefill_path=prefill_path, decode_path=decode_path
  ).run_prefill(
    params=params,
    tokens=prefill_input.tokens,
    cache=prefill_input.cache.cache,
